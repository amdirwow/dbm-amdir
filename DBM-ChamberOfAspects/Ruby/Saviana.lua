...
local warningWarnBeacon		= mod:NewTargetNoFilterAnnounce(74453, 4) -- Beacon targets are grouped briefly before announcing.
local warningWarnBreath		= mod:NewSpellAnnounce(74403, 3)

local specWarnBeacon		= mod:NewSpecialWarningYou(74453, nil, nil, nil, 1, 2) -- Aura application is the reliable trigger on this core.
...
local timerConflagCD		= mod:NewNextTimer(50, 74452, nil, nil, nil, 3) -- AzerothCore: Conflagration happens during the 50s flight cycle
local timerBreath			= mod:NewVarTimer("v20-30", 74403, nil, "Tank|Healer", nil, 5, nil, DBM_COMMON_L.TANK_ICON, true)
...
mod:GroupSpells(74453, 74456, 74452) -- Group target warning and follow-up debuff IDs.
...
function mod:OnCombatStart(delay)
	...
	timerBreath:Start(10-delay)
	timerFlight:Start(30-delay)
	table.wipe(beaconTargets)
	self.vb.beaconIcon = 8
	if self.Options.RangeFrame then
		DBM.RangeCheck:Show(12)
	end
	self:Schedule(29.5, savianaPhaseCatcher, self)
	self:Schedule(30, savianaAirphase, self)
end
...
function mod: UNIT_SPELLCAST_SUCCEEDED(_, spellName) -- SUCCEEDED is the stable event for Conflagration on this core.
	if spellName == GetSpellInfo(74454) then -- Conflagration
		timerConflagCD:Restart() -- Close to the server cycle; travel time can vary slightly.
		timerLanding:Start()
