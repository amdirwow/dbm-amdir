...
--    * enGB/enUS: Tandanu				https://amdir.in.ua
--    * deDE: Tandanu					https://amdir.in.ua
...
do
	...
	function DBT:CreateBar(timer, id, icon, huge, small, color, isDummy, colorType, inlineIcon, keep, fade, countdown, countdownMax)
		...
		if self.Options.VarianceEnabled then
			...
			timer = varianceMaxTimer -- Hidden variance bars should not hit zero at the minimum timer.
		end
		...
		if id == bar.id then
			...
			elseif type(totalTime) == "string" then -- found string (variance)
				...
				if self.Options.VarianceEnabled then
					...
					totalTime = varianceMaxTimer -- Keep hidden-variance bars aligned to the latest possible timer on refresh as well.
				end
...
function barPrototype:Update(elapsed)
	...
	local visibleTimerValue = timerCorrectedNegative
	if visibleTimerValue < 0 then
		visibleTimerValue = 0
	end
	...
	if timerValue <= 0 and not (barOptions.KeepBars and self.keep) and not (varianceBehaviorNeg and self.varianceDuration and (timerValue < -self.varianceDuration)) then
		...
		if fillUpBars then
			if currentStyle == "NoAnim" and timerValue <= enlargeTime and not enlargeHack and not self.varianceDuration then
				...
				bar:SetValue(math.max(0, math.min(1, 1 - timerValue/(totaltimeValue < enlargeTime and totaltimeValue or enlargeTime))))
			else
				bar:SetValue(math.max(0, math.min(1, 1 - timerValue/totaltimeValue)))
			end
		else
			if currentStyle == "NoAnim" and timerValue <= enlargeTime and not enlargeHack and not self.varianceDuration then
				-- Simple/NoAnim Bar mimics BW in creating a new bar on large bar anchor instead of just moving the small bar
				bar:SetValue(math.max(0, math.min(1, timerValue/(totaltimeValue < enlargeTime and totaltimeValue or enlargeTime))))
			else
