...
local timerGasSporeCD		= mod:NewVarTimer("v40-45", 69279, nil, nil, nil, 3, nil, nil, true)	-- Pull at 20-25s, then repeats roughly every 40-45s on AzerothCore/logs.
local timerPungentBlight	= mod:NewCDTimer(33.5, 69195, nil, nil, nil, 2)		-- Fires 33.5s after the third Inhaled Blight.
local timerInhaledBlight	= mod:NewVarTimer("v33.5-35.01", 69166, nil, nil, nil, 6, nil, nil, true)	-- Aura-based timing, usually 33.5-35s between stacks.
...
local timerGastricBloatCD	= mod:NewVarTimer("v15-17.5", 72219, nil, "Tank|Healer", nil, 5, nil, DBM_COMMON_L.TANK_ICON) -- AzerothCore repeats Gastric Bloat every 15-17.5s after the first cast.
local timerGooCD			= mod:NewVarTimer("v15-20", 72297, nil, nil, nil, 3)
...
function mod:OnCombatStart(delay)
	...
	timerInhaledBlight:Start(sformat("v%s-%s", 25-delay, 30-delay)) -- Pull timing verified in ICC logs.
	timerGasSporeCD:Start(sformat("v%s-%s", 20-delay, 25-delay)) -- Pull timing verified in ICC logs.
	timerGastricBloatCD:Start(sformat("v%s-%s", 12.5-delay, 15-delay)) -- AzerothCore pull timing is 12.5-15s.
	table.wipe(gasSporeTargets)
	table.wipe(vileGasTargets)
	...
	if self:IsHeroic() then
		timerGooCD:Start(sformat("v%s-%s", 15-delay, 20-delay))
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(_, spellName)
	if spellName == GetSpellInfo(72299) then -- Malleable Goo Summon Trigger (10 player normal) (the other 3 spell ids are not needed here since all spells have the same name)
		DBM:AddMsg("Malleable Goo Summon Trigger UNIT_SPELLCAST_SUCCEEDED unhidden from combat log. Report this to Amdir on Discord")
		specWarnGoo:Show()
