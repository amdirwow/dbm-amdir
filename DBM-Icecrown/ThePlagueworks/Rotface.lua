...
local timerWallSlime			= mod:NewNextTimer(25, 69789) -- First pipe cycle is earlier than the regular rhythm.
local timerSlimeSpray			= mod:NewNextTimer(20, 69508, nil, nil, nil, 3) -- Usually 20s; rare drift self-corrects on the next cast.
...
local timerVileGasCD			= mod:NewVarTimer("v15-20", 72272, nil, nil, nil, 3) -- Heroic reschedule window seen on ICC logs.
...
function mod:OnCombatStart(delay)
	timerWallSlime:Start(8-delay) -- AzerothCore schedules the first Ooze Flood at 8s.
	timerSlimeSpray:Start(20-delay) -- First spray matches the normal 20s cycle.
	if self:IsHeroic() then
		timerVileGasCD:Start(15-delay) -- First heroic Vile Gas starts at 15s.
	end
	self.vb.InfectionIcon = 1
	spamOoze = 0
	if self.Options.RangeFrame and self:IsHeroic() then
		DBM.RangeCheck:Show(10) -- 10 yards keeps heroic Vile Gas safe.
	end
...
function mod:SPELL_CAST_SUCCESS(args)
	if args:IsSpellID(72272, 72273) then
		DBM:AddMsg("Vile Gas SPELL_CAST_SUCCESS unhidden from combat log. Report this to Amdir on Discord")
		timerVileGasCD:Start()
	end
end
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	if (msg == L.YellSlimePipes1 or msg:find(L.YellSlimePipes1)) or (msg == L.YellSlimePipes2 or msg:find(L.YellSlimePipes2)) then
		timerWallSlime:Start() -- Restart on each pipe yell.
	end
end
