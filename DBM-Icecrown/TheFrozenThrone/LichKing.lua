...
mod:RegisterEventsInCombat(
	...
	"UNIT_DIED",
	"UNIT_SPELLCAST_SUCCEEDED" -- Unfiltered because boss units can desync on some private cores.
)

-- Shadow Trap is reliable enough with UNIT_TARGET.
-- Defile still needs boss target scanning; UNIT_TARGET misses too many casts.
...
local timerInfestCD					= mod:NewCDCountTimer(22.5, 70541, nil, "Healer|RaidCooldown", nil, 5, nil, DBM_COMMON_L.HEALER_ICON, true) -- AzerothCore: EVENT_INFEST rescheduled at 22.5s
...
local timerNecroticPlagueCD			= mod:NewCDTimer(30, 70337, nil, nil, nil, 3, nil, DBM_COMMON_L.DISEASE_ICON, true) -- Observed around 30-33s.
local timerEnrageCD					= mod:NewCDCountTimer("d20", 72143, nil, "Tank|RemoveEnrage", nil, 5, nil, DBM_COMMON_L.ENRAGE_ICON--[[, true]]) -- Can be skipped if Shambling Horror is stunned on precast.
...
local timerTrapCD					= mod:NewNextTimer(15.5, 73539, nil, nil, nil, 3, nil, DBM_COMMON_L.DEADLY_ICON, nil, 1, 4) -- Fixed on heroic.
...
local timerSoulreaperCD				= mod:NewCDCountTimer(32.5, 69409, nil, "Tank|Healer|TargetedCooldown", nil, 5, nil, DBM_COMMON_L.TANK_ICON)
local timerDefileCD					= mod:NewCDCountTimer(32.5, 72762, nil, nil, nil, 3, nil, DBM_COMMON_L.DEADLY_ICON, true, 1, 4) -- AzerothCore base reschedule is 32.5s; longer gaps come from server-side Defile/Valkyr/Frostmourne rescheduling. Verified in /wow/Logs/Lich.txt.
local timerSummonValkyr				= mod:NewCDCountTimer(45, 69037, nil, nil, nil, 1, 71844, DBM_COMMON_L.DAMAGE_ICON, true, 2, 3) -- AzerothCore: EVENT_SUMMON_VALKYR rescheduled at 45s
...
local timerSoulShriekCD				= mod:NewVarTimer("v12-15", 69242, nil, nil, nil, 1) -- Observed 12-15s between casts.
...
local function NextPhase(self, delay)
	...
	if self.vb.phase == 1 then
		...
		timerNecroticPlagueCD:Start(-delay) -- Same opening timer on normal and heroic.
	...
	elseif self.vb.phase == 2 then
		...
		timerSummonValkyr:Start(20, self.vb.valkyrWaveCount+1) -- First valkyr wave at 20s in phase 2.
		...
		timerInfestCD:Start(14, self.vb.infestCount+1) -- First phase 2 infest after 14s.
	...
	elseif self.vb.phase == 3 then
		...
		timerHarvestSoulCD:Start(14) -- First phase 3 Harvest Soul after 14s.
--		if self:IsHeroic() then
--			self:RegisterShortTermEvents(
--				"ZONE_CHANGED"
--			)
...
local function leftFrostmourne(self)
	...
	timerHarvestSoulCD:Start(58.72) -- Exit Frostmourne to next Harvest Soul cast.
	...
	timerVileSpirit:Start(7.81) -- Exit Frostmourne to next Vile Spirits cast.
end
...
function mod:SPELL_CAST_START(args)
	...
	if args:IsSpellID(68981, 74270, 74271, 74272) or args:IsSpellID(72259, 74273, 74274, 74275) then -- Remorseless Winter (phase transition start)
		...
		timerPhaseTransition:Start()
			timerRagingSpiritCD:Start(4, self.vb.ragingSpiritCount) -- AzerothCore: first intermission summon is scheduled 4s after transition starts
	...
	elseif spellId == 72762 then -- Defile
		...
		warnDefileSoon:Cancel()
		warnDefileSoon:CancelVoice()
		warnDefileSoon:Schedule(27.5, self.vb.defileCount+1)
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif args:IsSpellID(69409, 73797, 73798, 73799) then -- Soul reaper (MT debuff)
		...
		soundSoulReaperSoon:Schedule(32.5-2.5, "Interface\\AddOns\\DBM-Core\\sounds\\RaidAbilities\\soulreaperSoon.mp3")
		if args:IsPlayer() then
...
--[[		self.vb.valkyrWaveCount = self.vb.valkyrWaveCount + 1
	...
	elseif spellId == 70372 then -- Shambling Horror
		...
		timerEnrageCD:Start(12.3, shamblingCount, args.destGUID) -- First possible Shambling Enrage after summon.
		timerEnrageCD:Schedule(12.3+2, nil, shamblingCount, args.destGUID) -- Recheck in case the first cast is delay/stun-affected.
	end
end
